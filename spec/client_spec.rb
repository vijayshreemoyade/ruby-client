...
describe BitPay::SDK::Client do
  ...
  before do
    ...
    stub_request(:get, %r{#{BitPay::TEST_API_URI}\/tokens.*})
    ...
    stub_request(:delete, "#{BitPay::TEST_API_URI}/nuttin")
      .to_return(body: get_fixture('response-nodata.json'))
    stub_request(:get, "#{BitPay::TEST_API_URI}/settlements?token=MERCHANT_TOKEN")
      .to_return(body: get_fixture('settlements-GET.json'))
    stub_request(:get, "#{BitPay::TEST_API_URI}/settlements?token=MERCHANT_TOKEN&startDate=2019-12-01&endDate=2020-12-01&currency=USD&limit=1")
      .to_return(body: get_fixture('settlements-GET.json'))
    stub_request(:get, "#{BitPay::TEST_API_URI}/settlements/TEST_SETTLEMENT_ID?token=MERCHANT_TOKEN")
      .to_return(body: get_fixture('settlements_{id}-GET.json'))
    stub_request(:get, "#{BitPay::TEST_API_URI}/settlements/TEST_SETTLEMENT_ID/reconciliationreport?token=TEST_SETTLEMENT_TOKEN")
      .to_return(body: get_fixture('settlements_{id}_reconciliationreport-GET.json'))
  end
  ...
  describe '#get_settlements' do
    subject { bitpay_client }
    before { stub_const('ENV', 'BITPAY_PEM' => PEM) }

    it { is_expected.to respond_to(:get_settlements) }

    it 'gets merchant\'s token' do
      bitpay_client.get_settlements
      expect(WebMock).to have_requested :get, "#{BitPay::TEST_API_URI}/settlements?token=MERCHANT_TOKEN"
    end

    it 'correctly handles additional params' do
      bitpay_client.get_settlements(params: { startDate: '2019-12-01', endDate: '2020-12-01', currency: 'USD', limit: 1 })
      expect(WebMock).to have_requested(
        :get,
        "#{BitPay::TEST_API_URI}/settlements?token=MERCHANT_TOKEN&startDate=2019-12-01&endDate=2020-12-01&currency=USD&limit=1"
      )
    end
  end
  ...
  describe '#get_settlement' do
    ...
    it { is_expected.to respond_to(:get_settlement) }

    it 'correctly make request' do
      bitpay_client.get_settlement(settlement_id: 'TEST_SETTLEMENT_ID')
      expect(WebMock).to have_requested(
        :get,
        "#{BitPay::TEST_API_URI}/settlements/TEST_SETTLEMENT_ID?token=MERCHANT_TOKEN"
      )
    end
  end

  describe '#get_reconciliation_report' do
    subject { bitpay_client }
    before { stub_const('ENV', 'BITPAY_PEM' => PEM) }

    it { is_expected.to respond_to(:get_reconciliation_report) }

    it 'correctly make request' do
      bitpay_client.get_reconciliation_report(
        settlement_id: 'TEST_SETTLEMENT_ID',
        settlement_token: 'TEST_SETTLEMENT_TOKEN'
      )
      expect(WebMock).to have_requested(
        :get,
        "#{BitPay::TEST_API_URI}/settlements/TEST_SETTLEMENT_ID/reconciliationreport?token=TEST_SETTLEMENT_TOKEN"
      )
    end
  end
